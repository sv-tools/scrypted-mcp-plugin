...
- DCR registrations live in plugin storage under `oauth.client.<id>`. We accept any registration (PKCE-only public clients).
...
- **Origin auto-detection.** `originFromRequest` in `oauth.ts` infers protocol from the inbound port: matches `SCRYPTED_INSECURE_PORT` (default 11080) → http, anything else → https. Trusts `x-forwarded-proto` first when behind a reverse proxy. Getting this wrong is fatal — every metadata fetch fails and the SDK falls back to `${origin}/register` at the host root.
...
- `/authorize` auto-approves for any logged-in Scrypted user (option a from the design discussion). No consent screen.
